...
| Tema escuro | **O mesmo cartaz, impresso ao contrário** | Nenhuma cor da marca se move: `#690404`, `#5D0404` e `#FFCC00` ficam onde estavam, e papel e tinta trocam de lugar. Metade do site já era escura — 12 faixas vermelhas, 15 profundas, cabeçalho, rodapé e toda a camada fotográfica — e é essa metade que fica intacta, com os contrastes já medidos. Escolhido por painel de quatro paletas e três juízes; os três escolheram esta. |
| Acento no escuro | **Amarelo, não vermelho** | Sobre `#161210` o vermelho da marca dá **1,44:1**. Sumiriam todos os olhos-de-seção, anos da linha do tempo, sublinhas de link e — o que importa — o anel de foco. O amarelo dá **12,31:1** e é a outra cor da casa, não uma cor nova. |
| A faixa amarela no escuro | **Campo vermelho, tipo amarelo** | Um campo inteiro de `#FFCC00` à noite ofusca. A faixa é impressa ao contrário: `#FFCC00` sobre `#690404`, **8,57:1** — a inversão exata da faixa vermelha. O modo escuro automático que o usuário testou virou esse trecho em oliva com tipo rosa; o erro foi girar o matiz de 48° para 51°, o caminho do verde. Escurecer o amarelo da casa gira para 41°, o ocre. Se um dia alguém quiser campo claro aqui, é para esse lado que se anda. |
| Costuras entre faixas | **Filete sólido de `#FFCC00`** | Com o fundo em `#161210` o vermelho fica a 1,44:1 dele e o profundo a 1,32:1 — as faixas perderiam a borda. O filete dá 12,31:1 contra o fundo, 8,57:1 contra o vermelho, 9,36:1 contra o profundo. 2px nas faixas, 4px na amarela, no cabeçalho e no rodapé. |
| Placas de marca no escuro | **Filete amarelo por dentro** | Os fundos das sete placas são assados no arquivo e não seguem o tema. Três são escuras — azul-marinho 1,04:1, verde 1,31:1, carmim 1,83:1 contra o fundo — e sumiriam dentro do card. `inset 0 0 0 1px rgba(255,204,0,.62)` devolve a borda: 5,26 / 4,46 / 3,34:1 contra o interior de cada uma. Nas claras quase não aparece, e não precisa. |
| Como o tema é aplicado | **`data-theme` no `<html>`, sempre** | Um `<script>` inline e síncrono no `<head>` escreve o atributo antes da primeira pintura — a escolha guardada em `localStorage` (`pg-tema`), ou o que `prefers-color-scheme` pedir. Por isso a CSS **não tem** bloco `@media (prefers-color-scheme)`: seriam dois blocos para manter em sincronia e um envelheceria. Sem JavaScript o site fica no tema claro, que é completo — meio tema escuro seria pior que nenhum. |
| Cor em estilo de linha | **Nunca** | O fecho de /quem-somos saía com `style="color:var(--pg-red)"`, e estilo em linha vence qualquer regra de tema: no escuro ele ficava vermelho a **1,44:1** sobre o fundo escuro, quase ilegível. Virou a classe `.manifesto--accent`, que lê `--band-accent` — vermelho no claro (11,81:1), amarelo no escuro (12,31:1). É a segunda vez que cor em linha causa isso neste repositório; a primeira foi a placa dos cards. |
| Filete das placas no escuro | **`outline`, nunca `box-shadow: inset`** | Sombra interna pinta na camada de fundo do elemento, **abaixo dos filhos** — e a arte da placa é um `<img>` de `object-fit: cover` que cobre a caixa inteira. Escrito com `box-shadow`, o filete era computado e nunca rasterizado: as três placas escuras ficavam a 1,04, 1,33 e 1,84:1 do fundo, exatamente o defeito que a regra existia para corrigir. Medido: com sombra, a borda do FLACA lia `(0,12,42)`, a arte pura; com `outline` + `outline-offset: -1px` lê `(158,130,16)`. O verificador reprova se o `box-shadow` voltar. |
| Barra do navegador ao escolher tema | **Reescrever as duas `<meta>`, não acrescentar uma terceira** | O `<head>` traz duas `theme-color` com `media`, uma por esquema do sistema, e o navegador usa a **primeira cujo media casa** — uma terceira no fim nunca vence por ordem. Quando alguém escolhe contra o sistema, `main.js` reescreve as duas existentes e remove o `media`. Sem isso, quem tem o celular no escuro e escolhe claro lia uma página de papel sob uma barra quase preta. |
| Voltar do cache de retrocesso | **`pageshow` com `event.persisted`** | Voltar para uma página restaurada não roda script nenhum: o `<html>` volta com o `data-theme` que tinha quando a pessoa saiu. Quem trocava o tema e apertava "voltar" encontrava o tema antigo, com o `localStorage` dizendo outra coisa. Num site de 21 páginas, o botão Voltar é a forma mais comum de sair de uma página. |
| Impressão a partir do escuro | **`color-scheme: light` e costuras zeradas** | `body { background: #fff }` não basta: `color-scheme: dark` segue valendo e o navegador pinta a **tela do papel** de escuro — a folha saía com moldura preta em volta. E as costuras amarelas do tema escuro viravam tinta gasta numa folha onde todos os campos já são brancos. Conferido: a página impressa é **idêntica pixel a pixel** nos dois temas. |
| Botão dentro de `.prose` | **`.prose a.btn { text-decoration: none }`** | `.prose a` tem especificidade maior que `.btn` e reimpunha a sublinha por cima do rótulo. No claro ela era vermelha sobre botão vermelho e não se via; no escuro, com o acento da faixa invertida em branco, virava um risco branco cortando o CTA de /contato — o "nunca branco sobre amarelo" que o cabeçalho da seção 9 proíbe. |
| A cor do fundo escuro | **Escrita em três arquivos, travada no verificador** | `#161210` está na CSS (`--pg-paper` do bloco escuro), em `tools/build-site.py` (`THEME_COLOR_DARK`, que vira a `<meta theme-color>` das 21 páginas) e em `assets/js/main.js` (o valor que o botão grava). `check_theme()` cobra que os três sejam iguais — divergindo, o site fica com um fundo e a barra do navegador do celular com outro, e isso só aparece num aparelho. |
| Lista de projetos do rodapé | **Sai de `CATEGORIES`, não de uma lista à mão** | Todos os festivais e todos os cineclubes, na ordem do portfólio. Escrita à mão ela já tinha ficado para trás — Educa Ambiental e Marighella nunca chegaram nela — e voltaria a ficar no próximo projeto. As três ações de formação (#Abrindoacaixapreta, Arte para Voar, Oficinas de dança) ficam fora de propósito: com elas o rodapé teria dez itens contra quatro das outras colunas. Se a produtora quiser as dez, é trocar o filtro por `ORDER` inteiro. |
| Marca repetida na home | **O card de andamento herda o `sizes` do destaque** | FICA e FLACA aparecem duas vezes na home: como projeto, na grade de destaque, e como edição corrente, na de andamento. As duas grades têm larguras diferentes, e com `sizes` diferentes o navegador escolhia dois arquivos diferentes da MESMA marca e baixava a imagem duas vezes. Quando o projeto já está no destaque, o card de andamento declara o `sizes` do destaque e reaproveita o arquivo. Nada no build nem no verificador enxergaria isso — só medindo `currentSrc` no navegador. |
...
| Grão do Cine Retrata | **Desfocado só no fundo** | O grão do papel é ruído de alta frequência: sozinho triplicava o arquivo e some na tela no tamanho servido. `_smooth_ground()` desfoca com a marca protegida por máscara — WebP de 654 para 172 KB no 1500, sem tirar um fio da linha preta. |
| Telefone | **Publicado, sem nota** | Saiu e voltou em 7/9/2026. Os dois números estão no rodapé das 21 páginas e num bloco da página de contato chamado **"Contato"**, sem nota — a frase "Dois números da produtora" foi dispensada pela produtora. Segue em aberto qual é o principal e qual é WhatsApp, e por isso nenhum dos dois aparece como preferencial. |
| Valor de contato | **Dimensionado pela coluna, com `cqi`** | Era `var(--fs-h3)`, que cresce com a janela — mas a coluna para de crescer no teto da `.shell`. Em 1280px o tipo já tinha 21,6px e a coluna só 261, e aí "patriagrandeproducoes@" (265px) não cabia: o navegador caía no `overflow-wrap` e quebrava antes do arroba, ou deixava o "s" do @perfil sozinho na linha. Agora é `clamp(1.05rem, 7.6cqi, 1.35rem)` sobre `.contact__block { container-type: inline-size }` — a maior palavra do bloco tem 22 caracteres e mede ~12,25× o corpo, então 7,6cqi a deixa em 93% da coluna. Medido em Chrome de 505 a 2545 px: o @perfil nunca quebra, e o e-mail ou cabe inteiro ou quebra no arroba. |
| Quebra do e-mail | **`<wbr>` depois do arroba, e só ali** | `wbr()`, em `build-site.py`, marca a quebra depois do arroba quando há letra dos dois lados — o `@` de perfil fica intacto. Marcar o ponto também foi testado e descartado: dava "…@gmail." / "com". O `<wbr>` sozinho não bastava: se a primeira metade não coubesse, o navegador ignorava a marca e quebrava onde calhasse. Por isso ele anda junto com o corpo por `cqi`. |
